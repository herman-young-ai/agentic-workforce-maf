...
> No `/login` or `/refresh` — Entra ID handles token issuance externally. The platform maintains a local `User` table that maps to Entra ID Object IDs. `POST /admin/users` creates this platform record (auto-provisioned on first login via JIT). `reset-api-credential` resets the API key credential only, not Entra ID passwords.
...
| `PATCH` | `/{projectId}` | Update project fields (incl. `budget_ceiling` for budget extension) | Owner | |
...
### 1.6b Planner Board (`/api/v1/projects/{projectId}/board`)

| Method | Path | Purpose | Auth | Notes |
|--------|------|---------|------|-------|
| `GET` | `/` | Tasks grouped by status column with dependency edges | Viewer+ | Returns `BoardResponse` — optimised for Kanban rendering, avoids N+1 |
...
| `POST` | `/{learningId}/promote` | Propose promotion to platform | Operator+ | Creates a PromotionRequest record; sets learning `promotion_status=pending` |
...
### 1.13b Milestones (`/api/v1/projects/{projectId}/milestones`)

| Method | Path | Purpose | Auth | Notes |
|--------|------|---------|------|-------|
| `GET` | `/` | List milestones | Viewer+ | |
| `GET` | `/{milestoneId}` | Milestone detail with summary | Viewer+ | |
| `POST` | `/` | Create milestone manually | Operator+ | Idempotency-Key; also created by knowledge extractor |
...
| `POST` | `/{documentId}/retract` | Delete document (soft-delete via retract) | Owner | This IS the delete action in the UX; record retained for audit, content purged after retention period; no hard-delete endpoint (regulatory requirement) |
...
### 1.23b Templates — Browse (`/api/v1/templates`)

| Method | Path | Purpose | Auth | Notes |
|--------|------|---------|------|-------|
| `GET` | `/` | List available project + workflow templates | Member | Read-only; enables template discovery in project/workflow creation wizards |
| `GET` | `/{templateId}` | Template detail (composition, description) | Member | No admin-only fields exposed |
...
| `POST` | `/promotions/{promotionId}/approve` | Approve promotion | PlatformAdmin | `promotionId` = PromotionRequest ID (created by project-level `/promote`) |
| `POST` | `/promotions/{promotionId}/reject` | Reject promotion | PlatformAdmin | Resets source learning `promotion_status=rejected` |
...
public sealed record CreateProjectRequest
{
    ...
    [JsonPropertyName("jurisdiction")] public required string Jurisdiction { get; init; } // za_north, uk_south
...
public sealed record ProjectResponse
{
    ...
    [JsonPropertyName("objective")] public required string Objective { get; init; }
    [JsonPropertyName("description")] public string? Description { get; init; }
    [JsonPropertyName("jurisdiction")] public required string Jurisdiction { get; init; }
